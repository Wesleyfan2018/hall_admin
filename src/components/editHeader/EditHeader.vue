                ...
                <el-col :span="12" :offset="6">
                    <div class="text-right btn_group">
                        ...
                        <el-button
                            type="info"
                            @click="subimtAct(2)"
                            v-if="ButConf.hasOwnProperty('copyBut') && ButConf['copyBut'] ==1"
                        >复制配置</el-button>
                        <el-button
                            type="warning"
                            @click="subimtAct(3)"
                            v-if="ButConf.hasOwnProperty('syncBut') && ButConf['syncBut'] ==1"
                        >同步配置</el-button>
        ...
        >
            <section v-show="actionType == 1" class="publish">
                <el-form
                    ...
                    label-width="80px"
                    v-if="ButConf.hasOwnProperty('approver') && ButConf['approver'] ==1"
                >
                    ...
                    <el-form-item>
                        <el-button type="success" @click="apply()">申请审批</el-button>
                    </el-form-item>
                </el-form>
                <el-form
                    :inline="true"
                    ...
                    label-position="right"
                    label-width="80px"
                    v-if="ButConf.hasOwnProperty('approver') && ButConf['approver'] ==0"
                >
                    <el-form-item>
                        <el-button type="success" @click="officialSend()">确定发布</el-button>
                    </el-form-item>
                </el-form>
            ...
            <section v-show="actionType == 3" class="publish">
                ...
                <vue-code-diff
                    :old-string="syncoldStr"
                    :new-string="syncnewStr"
                    outputFormat="side-by-side"
                    :context="100"
                />
            </section>
...
export default {
    ...
    data() {
        return {
            ...
            thatValue: [], // 外部赋值方式给地区组件赋值
            initArea: [0] // 地区选项初始化默认值
        }
    },
    created() {
        this.getOptions()
    ...
    methods: {
        ...
        apply() {
            ...
            if (this.$parent.applySend) {
                this.$parent.applySend({ approverId: this.approverId })
            }
        },
        //不经过审批发布
        officialSend() {
            this.popVisble = false
            if (this.$parent.officialSend) {
                this.$parent.officialSend()
            }
        }, //
        // 弹框关闭
        closePop() {
        ...
        subimtAct(Num) {
            ...
            switch (Num) {
                case 1:
                    ...
                    this.oldStr = !this.sendData.oldJson
                        ? ''
                        : this.sendData.oldJson
                    this.newStr = !this.sendData.newJson
                        ? ''
                        : this.sendData.newJson
                    break
                case 2:
                ...
                case 3:
                    ...
                    this.dialog_width = '70%'
                    this.syncoldStr = !this.sendData.syncoldJson
                        ? ''
                        : this.sendData.syncoldJson
                    this.syncnewStr = !this.sendData.syncnewJson
                        ? ''
                        : this.sendData.syncnewJson
                    break
            }
            this.popVisble = true
        },
        ...
        LoadEnvConf() {
            ...
            for (let i in envInfo.list) {
                if (String(envInfo.list[i].env) === String(this.cEnvId)) {
                    this.envConf.push({
                        disabled: true,
                        label: envInfo.list[i].label,
                        env: envInfo.list[i].env
                    })
                } else {
        ...
        copyConf() {
            ...
            this.$confirm('确认复制？', '提示', {
                ...
                .then(() => {
                    this.$parent.copyConf({
                        channl: this.channelId,
                        new_channl: this.newChannelId,
                        area: this.areaId,
                        new_area: this.newAreaId,
                        env: this.envId
                    })
                })
                .catch(() => {})
