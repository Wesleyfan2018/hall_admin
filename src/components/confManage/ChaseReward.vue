    ...
    >
        ...
        <template v-if="formType==='add' && levelCount2>0">
            <ul class="tbody addType" v-for="i in levelCount2" :key="i">
                ...
                <li>
                    <input type="text" class="awardMoneyType" value />
                </li>
                <li>
                    <el-select
                        v-model="toolsArr[i]"
                        disabled
                        @change="changeEve($event, i, 'toolType')"
                    >
                        <el-option
                ...
                <li>
                    <input type="text" class="awardCount" value="0" :disabled="true" />
                </li>
            </ul>
        </template>
        <template v-if="formType==='add' && levelCount2===0">
        ...
        >
            <ul class="tbody addType" v-for="(item,index) in thatFormObj" :key="item.id">
                ...
                <li>
                    <el-select
                        v-model="toolsArr[index]"
                        disabled
                        class="form-input-min"
                        :readonly="inputReadOnly"
                        @change="changeEve($event, index, 'toolType')"
                    >
                        <el-option
                            v-for="item in toolsMap"
                            :key="item.value"
                ...
                <li>
                    <input :disabled="allLocked" type="text" class value :readonly="inputReadOnly" />
                </li>
            </ul>
        </template>
    </div>
    ...
    methods: {
        changeEve(val, index, field) {
            if (field === 'currType') {
                ...
                this.$set(this.toolsArr, index, val);
            }
        },
        // formType = 'add' 时，表单提交
        SetSubmitEve(type) {
            ...
            for (let i = 0; i < for_length; i++) {
                ...
                let awardCount = 0;
                if (type === 'add') {
                    awardMoneyType = this.currTypeArr[i + 1];
                } else {
                    awardMoneyType = this.currTypeArr[i]; // 奖励类型
                }
