            ...
            <el-form :inline="false" class="demo-form-inline">
                ...
                <el-form-item>
                    <div class="tag-group text-center" style="width:90%;margin:0 auto;">
                        <el-tag v-show="idx !== 0" v-for="(item, idx) in tagList" :key="(item, idx)" :effect="idx == currentTag ? 'dark' : 'plain'" @click="changeTag(idx)">
                ...
                <el-table stripe border :data="tableData" v-loading="loading">
                    ...
                    <el-table-column align="center" label="新增">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 2 && idx > 0" :key="(item, idx)" align="center" :prop="item.name">
                    ...
                    <el-table-column align="center" label="活跃人数">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 5 && idx > 2" :key="(item, idx)" align="center" :prop="item.name">
                                <template slot="header" :slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" :content="item.title" placement="top">
                                        <span>{{item.zh}}</span>
                                    </el-tooltip>
                    ...
                    <el-table-column align="center" label="日收入">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 10 && idx > 5" :key="(item, idx)" align="center" :prop="item.name">
                                <template slot="header" :slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" :content="item.title" placement="top">
                                        <span>{{item.zh}}</span>
                                    </el-tooltip>
                    ...
                    <el-table-column align="center" label="留存">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 15 && idx > 10" :key="(item, idx)" align="center" :prop="item.name">
                                <template slot="header" :slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" :content="item.title" placement="top">
                                        <span>{{item.zh}}</span>
                                    </el-tooltip>
                    ...
                    <el-table-column align="center" label="活跃度">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 20 && idx > 15" :key="(item, idx)" align="center" :prop="item.name">
                                <template slot="header" :slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" :content="item.title" placement="top">
                                        <span>{{item.zh}}</span>
                                    </el-tooltip>
                    ...
                    <el-table-column align="center" label="新增用户数据">
                        <template v-for="(item, idx) in tagList">
                            <el-table-column v-if="idx <= 27 && idx > 20" :key="(item, idx)" align="center" :prop="item.name">
                                <template slot="header" :slot-scope="scope">
                                    <el-tooltip class="item" effect="dark" :content="item.title" placement="top">
                                        <span>{{item.zh}}</span>
                                    </el-tooltip>
        ...
        return {
            ...
            tagList:
                [
                    ...
                    { 'zh': '日新增', 'name': 'register_num', 'format': '', 'title': '统计当日，首次注册登录的新用户（ID 去重）' },
                    { 'zh': '手机账号', 'name': 'register_phone_num', 'format': '', 'title': '统计当日，首次注册登录的新用户（ID 去重）当中用手机号注册的用户数' },
                    { 'zh': '日登陆', 'name': 'login_num', 'format': '', 'title': '统计当日，有发生登录行为一次以上的用户数，ID 去重' },
                    { 'zh': 'DAU', 'name': 'dau', 'format': '', 'title': '统计当日，除去当天新增用户后的有发生登录行为一次以上的用户数，ID 去重' },
                    { 'zh': '累积付费用户数', 'name': 'acm_recharge_num', 'format': '', 'title': '统计当日，登录的用户当中有过付费行为的用户数' },
                    { 'zh': '日收入', 'name': 'recharge_amount', 'format': 'float', 'title': '统计当日，所有玩家充值的总金额' },
                    { 'zh': '付费人数', 'name': 'recharge_num', 'format': '', 'title': '统计当日，有成功付费的总人数' },
                    { 'zh': '付费渗透率', 'name': 'pay_seep_rate', 'format': 'percent', 'title': '统计当日，付费人数\/当天登录的用户数' },
                    { 'zh': 'arppu', 'name': 'arppu', 'format': 'float', 'title': '统计当日，付费总额\/付费用户数' },
                    { 'zh': 'arpu', 'name': 'rarpu', 'format': 'float', 'title': '统计当日，付费总额\/登录用户数' },
                    { 'zh': '30天回流人数', 'name': 'return30', 'format': '', 'title': '统计当日，流失 30 天后又回来的用户' },
                    { 'zh': '新增次日留', 'name': 'reg_remain2', 'format': 'percent', 'title': '统计当日的新增用户中，在统计日的第 2 日时有过登录日志的用户占比' },
                    { 'zh': '新增7日留', 'name': 'reg_remain7', 'format': 'percent', 'title': '统计当日的新增用户中，在统计日的第 7 日时有过登录日志的用户占比' },
                    { 'zh': '登录次日留', 'name': 'login_remain2', 'format': 'percent', 'title': '统计当日的登录用户中，在统计日的第 2 日时有过登录日志的用户占比' },
                    { 'zh': '登陆7日留', 'name': 'login_remain7', 'format': 'percent', 'title': '统计当日的登录用户中，在统计日的第 7 日时有过登录日志的用户占比' },
                    { 'zh': '最高在线人数', 'name': 'max_online_num', 'format': '', 'title': '统计当日，24 小时内同时在线最高达到的人数' },
                    { 'zh': '平均在线人数', 'name': 'avg_online_num', 'format': 'int', 'title': '统计当日，24 小时每小时同时在线相加总和\/24 小时' },
                    { 'zh': '玩牌人数', 'name': 'play_num', 'format': 'int', 'title': '统计当日，有过玩牌行为的人数(ID去重)' },
                    { 'zh': '未玩牌人数', 'name': 'not_play_num', 'format': 'int', 'title': '统计当日，登录了游戏但没有过玩牌行为的人数' },
                    { 'zh': '平均游戏时长(min)', 'name': 'avg_game_min', 'format': 'float', 'title': '统计当日，玩牌用户的玩牌时间\/玩牌人数' },
                    { 'zh': '新增日收入', 'name': 'register_income', 'format': 'float', 'title': '统计当日，新增用户充值的总金额' },
                    { 'zh': '新增付费人数', 'name': 'register_recharge_num', 'format': '', 'title': '统计当日，新增用户有成功付费的总人数' },
                    { 'zh': '新增付费渗透率', 'name': 'reg_pay_seep_rate', 'format': 'percent', 'title': '统计当日，新增用户当中付费人数\/当天新增用户数' },
                    { 'zh': '新增arppu', 'name': 'reg_arppu', 'format': 'float', 'title': '统计当日，新增用户的付费总额\/新增用户当中付费人数' },
                    { 'zh': '新增arpu', 'name': 'reg_arpu', 'format': 'float', 'title': '统计当日，新增用户的付费总额\/当天新增用户数' },
                    { 'zh': '新增玩牌人数', 'name': 'register_play_num', 'format': '', 'title': '统计当日，新增用户当中有过玩牌行为的人数' },
                    { 'zh': '新增平均游戏时长(min)', 'name': 'reg_avg_game_min', 'format': 'float', 'title': '统计当日，新增用户当中有过玩牌行为的用户的玩牌时间\/新增用户当中有过玩牌行为的玩牌人数' }
                ],
            // 导出所需表头
            tHeader: [],
            filterVal: [],
                ...
                if (res.code === 0) {
                    ...
                    for (let i in data) {
                        for (let t in data[i]) {
                            ...
                            let index = self.tagList.findIndex(idx => idx.name === t);
                            // 数据格式百分比处理
