        ...
        <div class="filter-container">
            <el-form :inline="true" class="demo-form-inline">
                ...
                <el-form-item label="地区：">
                    <el-select v-model="provId" placeholder="请选择">
                        <el-option v-for="item in provMap" :key="item.type" :label="item.name" :value="item.type"></el-option>
                    </el-select>
                </el-form-item>

                <el-form-item label="渠道：">
                    <el-select v-model="filterChannel" placeholder="请选择">
                        <el-option v-for="(cname, cid) in channelMap" :key="cid" :value="cid" :label="cname"></el-option>
                    </el-select>
                </el-form-item>
            ...
            <div class="table-main">
                <h4 id="ji-ben-yong-fa" style="color:#606266;margin-bottom:18px;">
                    总注册人数：<span class="el-rate__text" style="color: #409EFF">{{ registerCount }}</span>&nbsp;&nbsp;
                    总流水收入：<span class="el-rate__text" style="color: #409EFF">{{ chargeTotal }}</span>
                </h4>
                <el-table stripe border :data="tableData" v-loading="loading">
                    ...
                    <el-table-column align="center" label="新增">
                        ...
                        <el-table-column align="center" prop="register_play_rate">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，新增用户当中有过玩牌行为的用户数在新增用户当中的占比" placement="top">
                                    <span>玩牌率</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="register_avg_count">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，新增用户当中有过玩牌行为的用户的平均局数，公式=玩牌用户总局数/玩牌人数" placement="top">
                                    <span>平均玩牌局数</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                    ...
                    <el-table-column align="center" label="登录">
                        ...
                        <el-table-column align="center" prop="login_play_rate">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，登录用户当中有过玩牌行为的用户数在登录用户当中的占比" placement="top">
                                    <span>玩牌率</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="login_avg_count">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，登录用户当中有过玩牌行为的用户的平均局数，公式=玩牌用户总局数/玩牌人数" placement="top">
                                    <span>平均玩牌局数</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                    ...
                    <el-table-column align="center" label="充值">
                        <el-table-column align="center" prop="recharge_count">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，有过付费行为的用户数" placement="top">
                                    <span>充值总人数</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="recharge_amount">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，用户付费总额" placement="top">
                                    <span>充值总金额</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                    ...
                    <el-table-column align="center" label="提现">
                        <el-table-column align="center" prop="draw_count">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，有过提现行为的用户数" placement="top">
                                    <span>提现总人数</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="draw_amount">
                            <template slot="header" :slot-scope="scope">
                                <el-tooltip class="item" effect="dark" content="统计当日，用户提现总额" placement="top">
                                    <span>提现总金额</span>
                                </el-tooltip>
                            </template>
                        </el-table-column>
        ...
        getTableData() {
            ...
            revoke('index.php?m=stat&p=getRealBrief', data).then(res => {
                if (res.code === 0) {
                    ...
                    // 表格数据格式format
                    for (let i in list) {
                        let r_play_rate = list[i]['register_play_count'] / list[i]['register_count'];
                        let r_avg_count = list[i]['register_desk_count'] / list[i]['register_play_count'];
                        list[i]['register_play_rate'] = isNaN(r_play_rate) ? 0 : (r_play_rate * 100).toFixed(2) + '%';
                        list[i]['register_avg_count'] = isNaN(r_avg_count) ? 0 : r_avg_count.toFixed(2);

                        let l_play_rate = list[i]['login_play_count'] / list[i]['login_count'];
                        let l_avg_count = list[i]['login_desk_count'] / list[i]['login_play_count'];
                        list[i]['login_play_rate'] = isNaN(l_play_rate) ? 0 : (l_play_rate * 100).toFixed(2) + '%';
                        list[i]['login_avg_count'] = isNaN(l_avg_count) ? 0 : l_avg_count.toFixed(2);
                    }
                    self.tableData = list;
                    self.loading = false;
                    loading.close();
