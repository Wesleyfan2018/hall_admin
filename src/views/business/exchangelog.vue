    ...
    <div class="app-containei">
        <div class="filter-container">
            ...
            <el-table border :data="tableData" @cell-dblclick="ToUserinfo">
                ...
                <el-table-column align="center" label="玩家ID" prop="uid" class-name="a-hover"></el-table-column>
                <el-table-column align="center" label="玩家昵称" prop="nickName"></el-table-column>
                <el-table-column align="center" label="商品类型" prop="goodsType"></el-table-column>
                <el-table-column align="center" label="商品数量" prop="goodsNum"></el-table-column>
                <el-table-column align="center" label="商品名称" prop="goodsName" width="170"></el-table-column>

                <el-table-column align="center" label="消耗" prop="consumeNum"></el-table-column>

                <el-table-column align="left" label="附加信息" prop="extInfo" width="250"></el-table-column>

                <el-table-column align="center" label="状态" prop="statusName"></el-table-column>
                <el-table-column align="center" label="兑换时间" prop="time"></el-table-column>
                <el-table-column align="center" label="操作">
                    <template slot-scope="scope">
                        <el-button
                            size="mini"
                            v-if="tableData[scope.$index].status==1 && (tableData[scope.$index].type==3 || tableData[scope.$index].type==4) "
                            type="primary"
                            @click="caterr(scope.row)"
                        >原因</el-button>
                        <el-button
                            size="mini"
                            v-if=" (tableData[scope.$index].type==3 && tableData[scope.$index].status==1) || (tableData[scope.$index].type==4 && (tableData[scope.$index].status==1||tableData[scope.$index].status!=5 && tableData[scope.$index].phonefeeInfo.status!=1))  "
                            type="primary"
                            @click="refund(scope.row)"
                        >退款</el-button>
                        <el-button
                            size="mini"
                            v-if="tableData[scope.$index].status==3 && tableData[scope.$index].type==5 "
                            type="primary"
                            @click="shipBut(scope.row)"
                        >发货</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <div class="dialog_div">
            <el-dialog title="发货单号" :visible.sync="shipVisble" width="30%">
    ...
    methods: {
                ...
                if (res.code === 0) {
                    ...
                    for (let i in this.tableData) {
                        ...
                        this.tableData[i].time =
                            ...
                            ':' +
                            t.getSeconds();
                        this.tableData[i].consumeNum =
                            this.tableData[i].consumeNum +

                            this.tableData[i].consumeTypeName;
        ...
        ship() {
            let args = {
                ...
                expressType: this.selectData.expressType,
                expressNo: this.selectData.expressNo
            };
            let data = {
                callm: 'Exchangelog',
                callp: 'ship',
