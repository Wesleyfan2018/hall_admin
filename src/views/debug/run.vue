<template>
    <div class="app-containei">
        <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="php执行"></el-form-item>
            ...
            <el-table :data="phpRunLog" style="width: 100%" @cell-click="SelectLog">
                <el-table-column prop="cmd" label="历史记录"></el-table-column>
                <el-table-column label="收藏">
                    <template slot-scope="scope">
                        <el-button
                            class="filter-item"
                            type="primary"
                            :icon="scope.row.collectIcon"
                            size="medium"
            ...
            revoke('/index.php?m=debug&p=getPhpRunLog', data).then(res => {
                if (res.code === 0) {
                    for (let i in res.data.phpRunLog) {
                        res.data.phpRunLog[i].collectIcon =
                            res.data.phpRunLog[i].collect === '1'
                                ? 'el-icon-star-on'
                                : 'el-icon-star-off'
                    }
