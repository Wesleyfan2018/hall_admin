<template>
    <div class="app-containei">
        <el-form :inline="true" class="demo-form-inline">
            ...
            <el-form-item label="服务器：">
                <el-select v-model="ip" placeholder="请选择服务器" class="nav-select">
                    <el-option v-for="(i, t) in ips" :key="(i, t)" :label="i" :value="t"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-table
            border
            :data="loglist"
            :default-sort="{prop: 'allNum', order: 'descending'}"
            @cell-click="dispChart"
        >
            <el-table-column align="center" label="接口名称" prop="api"></el-table-column>
            <el-table-column align="center" label="请求数" sortable prop="allNum"></el-table-column>
            <el-table-column align="center" label="平均流入流量(kb)" sortable prop="allInput"></el-table-column>
            <el-table-column align="center" label="平均流出流量(kb)" sortable prop="allOutput"></el-table-column>
            <el-table-column align="center" label="平均运行时间(秒)" sortable prop="allRunTime"></el-table-column>
        </el-table>
    ...
    methods: {
            ...
            revoke('/index.php?m=CallProxy&p=callCommon', data).then(res => {
                if (res.code === 0) {
                    ...
                    for (let i in tableData) {
                        tableData[i].allRunTime =
                            Math.floor(
                                (tableData[i].allRunTime /
                                    tableData[i].allNum) *
                                    10000
                            ) / 10000
                        tableData[i].allInput =
                            Math.floor(
                                (tableData[i].allInput /
                                    tableData[i].allNum /
                                    1024) *
        ...
        _dispChart() {
            ...
            for (let i = 1; i <= 288; i++) {
                // 计算横坐标数据
                let temp =
                    i * 300 * 1000 + new Date().getTimezoneOffset() * 60 * 1000
                xdata.push(
                    new Date(temp).getHours() +
                        ':' +
                        new Date(temp).getMinutes()
                )
                // 计算纵坐标数据
                for (let j in this.chartfeild) {
                    ...
                    if (this.chartdata[this.chartfeild[j]][i]) {
                        switch (clickFeild) {
                            case 'allRunTime':
                                sdatatemp = this.chartdata[this.chartfeild[j]][
                                    i
                                ].runtime
                                break
                            case 'allNum':
